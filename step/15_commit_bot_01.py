...
# -------------------------------
# 2. 인코더 (Encoder) – d_model=32, nhead=4
# -------------------------------
...
# -------------------------------
# 3. 디코더 (Decoder) – d_model=32, nhead=4
# -------------------------------
...
class Seq2SeqTransformer(nn.Module):
    def __init__(self, src_vocab, tgt_vocab, d_model=32, nhead=4):
        ...
        # Transformer에 적합한 작은 학습률
         # 계속 0.28 이상이면→ lr=0.0001로 낮춰서 재학습 (너무 큰 걸음으로 최적점을 지나치는 걸 방지)
    ...
    def train_step(self, src_ids, tgt_ids):
        self.train()
        # 패딩 마스크를 float으로 변환 (MultiheadAttention 요구사항)
...
if __name__ == "__main__":
    ...
    JSONL_PATH = os.path.join(project_root, "data", "commit_dataset_10000_refined.jsonl")
    ...
    # ── 학습 루프 ──
    N_EPOCHS = 1000          # 10,000건이면 300~500 에폭으로 충분

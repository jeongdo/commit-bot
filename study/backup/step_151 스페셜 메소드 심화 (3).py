...
p2 = Point2(20, 40)
p3 = Point3(45, y=20)
p4 = Point4(10, 20, 30, 40)  # rename 설정으로 난수된 값이 설정된다.
p5 = Point3(**temp_dict)
...
# _make() : 새로운 객체 생성
p4 = Point1._make(temp)  # 리스트를 기반으로 네임드 튜플로 캐스팅
...
print(p1._asdict(), p4._asdict())  # 네임드 튜플을 OrderedDict 딕셔너리로 변경
...
students2 = [Classes(rank, number)
             for rank in 'A B C D'.split()  # A 하나당 하위가 실행 되고, for number in [str(n)  for n in range(1, 21)]] 20번 반복하고.
             for number in [str(n)
                            for n in range(1, 21)]]

data = [n * m for n in range(2, 10) for m in range(1, 10) if (n * m) % 2]

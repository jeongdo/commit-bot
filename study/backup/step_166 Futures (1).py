...
# 파이썬의 GIL (global interpreter lock) : 두 개 이상의 스레드가 동시에 실행 될 때, 하나의 자원을 엑세스 하는 경우 문제점을 방지하기 위해 리소스 전체에 락이 걸리는 현상이 실행됨.
# 즉, 리소스 전체에 락이 걸리며 Context Switch(문맥 교환) 비용이 발생한다.

# GIL의 우회법으로 멀티프로세싱 사용 또는 CPython을 이용
...
def main():
    # Worker Count
    worker = min(10, len(WORK_LIST))    # 10, 4개 중에 MIN 작은 수 = 4
    print(worker)

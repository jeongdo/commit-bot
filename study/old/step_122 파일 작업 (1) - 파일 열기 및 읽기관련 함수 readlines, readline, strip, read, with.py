...
file_aba_path = os.path.abspath('../data/sample.txt')

print("============== readlines =======================")
file = open(file_aba_path, 'r')
...
while True:
    line = file.readline()
    if not line:  # if len(line) <= 0 or line == '':
        break
    print(line)
...
print(
    "=============== print()에 전달되는 문자열에 '\\n'가 포함되어 있기 때문에 개행(new line)이 출력 : 개행 제거 : strip() =======================")
...
with open(file_aba_path, 'r') as file:
    data = file.read()
    print(data)

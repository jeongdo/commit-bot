...
try:
    file = open(file_aba_path, 'wt', encoding='utf-8')
    # readline()는 파일에서 1줄을 읽고 그 결과를 리턴해줍니다.
    file.write('파이썬으로 파일을 작성하고 있습니다.')
    file.write('newline 문자로 개행해봅니다.\n')
    file.write('개행이 잘되었나요?')
finally:
    file.close()
...
file_aba_path = os.path.abspath('../data/sample_02.txt')

# writelines()는 List의 모든 텍스트를 파일에 저장합니다.

str_list = ["hello\n", "python\n", "world"]
with open(file_aba_path, "w") as file:
